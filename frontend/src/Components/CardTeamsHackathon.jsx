...
export const CardTeamsHackathon = ({ team }) => {
  ...
  const handleClick = async (e) => {
    ...
    team.members.forEach((m) => {
      const cohort = m.hackathonMVPHistory.find((c) => c.hackathon == "3");
      if (cohort) {
  ...
  const handleDelete = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `http://localhost:5000/api/teams/removehackathon/${team._id}`,
        { member: e.target.id },
        {
          headers: { token: `Bearer ${token}` },
        }
